...
# PyTorch OpenPose 모델 로드
body_estimation = Body('C:/Users/DS/Desktop/kimsihyun/pytorch_openpose/model/body_pose_model.pth')
hand_estimation = Hand('C:/Users/DS/Desktop/kimsihyun/pytorch_openpose/model/hand_pose_model.pth')




def body(frame):
    """PyTorch 기반 신체 키포인트 감지 함수"""
    candidate, subset = body_estimation(frame)
    points = [None] * len(BODY_PARTS)
    confidences = [0.0] * len(BODY_PARTS)
    if subset.shape[0] > 0:
        for name, idx in BODY_PARTS.items():
            # idx가 subset[0]의 길이보다 크면 건너뜀
            if idx >= subset.shape[1]:
                continue
            kp_idx = int(subset[0][idx])
            if kp_idx >= 0 and kp_idx < candidate.shape[0]:
                x, y, score, _ = candidate[kp_idx]
                points[idx] = (int(x), int(y))
                confidences[idx] = float(score)
    return points, confidences

def hand(crop):
    """PyTorch 기반 손 키포인트 감지 함수"""
    peaks = hand_estimation(crop)
    points = []
    confidences = []
...
def process_frame(args):
    ...
    # =========================================
    points, body_confidences = body(filtered_frame)
    ...
    if points[BODY_PARTS["LWrist"]] is not None and points[BODY_PARTS["LElbow"]] is not None:
        ...
        if x1 > 0 and y1 > 0 and x2 < frameWidth and y2 < frameHeight:
            ...
            hand_left_points, hand_left_confidences = hand(filtered_crop)
    ...
    if points[BODY_PARTS["RWrist"]] is not None and points[BODY_PARTS["RElbow"]] is not None:
        ...
        if x1 > 0 and y1 > 0 and x2 < frameWidth and y2 < frameHeight:
            ...
            hand_right_points, hand_right_confidences = hand(filtered_crop)
...
def video_to_keypoints(cap, json_dir, filename):
    ...
    while cv.waitKey(1) < 0:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            batch_results = list(executor.map(process_frame, batch_frames))
...
if __name__ == "__main__":
    ...
    cap = cv.VideoCapture(video_path) #영상 파일
